...
function getAllKittenNumbers(totalSupply){

	var kittens = [];
	for(var x = 1; x < totalSupply; x++){
...
function separateByGeneration(){
	...
	console.log("Outputting generations");
}
...
function getAndSaveAllKittens(totalSupply){
	console.log("Total supply is: " + totalSupply);
	
	//For testing:
	//totalSupply = 500;
	
	kittens = getAllKittenNumbers(totalSupply);
	getOwnershipOfCatsLoop(kittens).then(getCatsLoop).then(separateByGeneration);
}

ck_contract.methods.totalSupply().call().then(getAndSaveAllKittens);
